...
- [ ] 等待 ready 时同时监听 `holder.exited`；若子进程提前退出，立即报告退出码、stdout 和 stderr。
- [ ] 在 PowerShell 脚本中记录脚本进入、开始打开文件、成功获得句柄、写入 ready、收到 release 和释放句柄等时间点。
- [ ] 分别为“等待 ready”“业务更新”“等待 holder 退出”设置诊断性阶段超时，并在任一阶段失败时输出完整状态；阶段阈值应在压力数据收集后确定。
- [ ] 清理时先请求 holder 退出并有界等待；必要时终止进程，确认退出后再使用带退避重试的临时目录删除。
- [ ] 记录 `powershell.exe` 路径与版本、Runner 镜像版本以及相关文件和锁路径状态。
- [ ] 在 Windows CI 上连续运行该测试至少 100 次，统计各阶段 P50、P95、最大值、提前退出次数和失败率，再判断应修复同步逻辑、子进程行为还是调整超时。
- [ ] 评估改用当前进程可直接控制的 Windows 句柄辅助程序，减少 PowerShell 这一额外变量；若增加测试辅助二进制，需要保证测试仍验证真实 Windows 独占句柄语义。
