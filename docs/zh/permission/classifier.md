...
| `builtin/bash@1` | `bash` 中的简单 npm 命令 |
...
`builtin/bash@1` 首期只处理一条简单命令，支持：

- 以空白分隔的命令名和参数；
- 单引号和双引号；
- 反斜杠转义；
- 可执行文件写成 `npm`、`npm.cmd`、`npm.exe`，也可带静态目录前缀。

以下情况直接弃权：

- 变量展开，例如 `$TARGET`、`${TARGET}`；
- 命令替换，例如 `$(command)` 或反引号；
- 引号或转义未闭合；
- 非 npm 命令；
- 尚未覆盖的 npm 子命令或参数形式。

当前分类器尚未拆分管道、`&&`、`||`、`;`、重定向、子 Shell 和控制结构，因此**仅应将单条简单 npm 命令视为已覆盖输入**。这些结构的解析和逐节点分类仍待实现；当前版本不能据此宣称已经覆盖复合命令的全部行为。
...
- bash、PowerShell、cmd 的独立白名单文法前端；
- Bash 结构性拒绝清单和复合命令逐节点分类；
- npm 之外的常见命令语义；
- 用户可配置的敏感路径列表；
- custom 策略文件加载；
- 应用级自定义分类器发现、启停和加载错误界面；
- 分类器执行超时；
- 独立轮转的 `permission-audit.jsonl`。
