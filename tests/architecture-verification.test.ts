...
/**
 * 架构可行性验证（对应项目路线图 M1 里程碑）。
 *
 * M1 的目标不是产品功能，而是回答一个一次性的问题：pi SDK、OpenTUI、
 * Photon 这几个第三方依赖，能不能一起编译进 Bun 单文件可执行程序，并在
 * 没有预装 Node/Bun 的机器上正常运行？下面四项检查分别验证这个问题的
 * 四个子命题；只要有一项失败，就说明技术选型存在问题，需要停下来重新
 * 决策，而不是继续在上面盖产品功能。
 *
 ...
 *    路线图"诉求 A/B"（每轮临时上下文注入、视图式提示词组织）将来要用
 *    到的机制，如果编译产物里失效，那两条核心定制诉求就无法实现。
