...
describe("TUI 参数", () => {
  test("接受数据目录和权限缺口收集开关", () => {
    expect(parseArguments(["--data-dir", ".aizen/dev-data", "--collect-permission-gaps"])).toEqual({
      command: "interactive",
      dataDirectory: ".aizen/dev-data",
      collectPermissionGaps: true,
    })
    expect(parseArguments(["--collect-permission-gaps", "--data-dir", ".aizen/dev-data"])).toEqual({
      command: "interactive",
      dataDirectory: ".aizen/dev-data",
      collectPermissionGaps: true,
    })
  ...
  test("没有参数时关闭收集并使用默认数据目录", () => {
    expect(parseArguments([])).toEqual({ command: "interactive", collectPermissionGaps: false })
  })
  ...
  test("用法包含所有可用启动参数与子命令", () => {
    ...
    expect(usage()).toContain("--collect-permission-gaps")
