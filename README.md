...
TUI 当前支持以下启动参数，参数顺序不限：

| 参数 | 行为 |
|---|---|
| `--data-dir <目录>` | 指定本地数据目录。相对路径以启动时的工作目录为基准；不能指定为当前工作目录。 |
| `--collect-permission-gaps` | 将权限脚本未覆盖或只能粗略判断的工具调用记录到本地，用于后续补充规则。 |

例如，从源码启动并收集权限规则缺口：

```powershell
bun run dev:tui -- --collect-permission-gaps
```

直接运行可执行程序时：

```powershell
.\aizen-tui.exe --data-dir .\data --collect-permission-gaps
```

启用 `--collect-permission-gaps` 后：

- 所有权限模式都会记录命中的规则缺口；常规模式继续使用原有验证结果，不重复运行验证器，也不改变 AI、人工审核或授权结论。
- “完全开放”模式会额外等待验证器完成，但验证器的允许、拒绝、升级审核或异常均不会阻止调用；该模式不调用审核 AI、不请求人工审核，并始终使用原始工具参数执行。
- 提权、敏感文件、工作区外操作和远程变更等明确的权限政策分支不属于规则缺口，不会仅因这些原因被记录。
- Bash 记录完整命令；文件工具只记录路径、内容规模或替换规模，不记录文件正文。第三方工具参数沿用权限审计的脱敏和截断规则。
- 记录写入 `<数据目录>/local-observations/permission-gaps.jsonl`。写入失败会报告错误，但不会改变工具授权结果。

不传该参数时，权限流程和本地文件均保持现有行为。
